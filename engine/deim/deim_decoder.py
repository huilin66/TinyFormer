...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
                target, # topk query
                ref_points_unact, # topk query boxes before sigmoid 
                memory, # all feature maps from encoder
                spatial_shapes,
...
class DEIMTransformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 eps=1e-2,
                 aux_loss=True,
                 cross_attn_method='default', #default
                 query_select_method='default', #default
                 reg_max=32,
                 reg_scale=4.,
                 layer_scale=1,
                 mlp_act='relu',
    ...
    def forward_from_memory(self, memory, spatial_shapes, targets=None):
        ...
        out_bboxes, out_logits, out_corners, out_refs, pre_bboxes, pre_logits = self.decoder(
            init_ref_contents,# topk query
            init_ref_points_unact, # topk query boxes before sigmoid  
            memory, # all feature maps from encoder
            spatial_shapes,
            self.dec_bbox_head,
            self.dec_score_head,
        ...
        if self.training and self.aux_loss:
            ...
            # Change: Pass 'k' (num_queries) to loss
            out['enc_meta'] = {
